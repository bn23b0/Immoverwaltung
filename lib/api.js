...
export const listTenanciesX = (token, propertyId = null) =>
  readWithCache(
    'tenancies' + (propertyId ? '_' + propertyId : ''),
    'list_tenancies',
    { p_token: token, p_property_id: propertyId },
    []
  );

export const upsertTenancy = (token, data) =>
  mutate('upsert_tenancy', { p_token: token, p_data: data });
export const deleteTenancy = (token, id) =>
  mutate('delete_tenancy', { p_token: token, p_id: id });
